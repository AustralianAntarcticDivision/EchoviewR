...
#' }
modifyECS=
  function (ECSPathAndFile, ECStargetVar, NewValue, maxNlines = 200, 
            nBlanks2Stop = 4, ECSPathAndFileOut) 
  {
    finder = function(ECSPathAndFile, maxNlines, ECSVar, verbose = FALSE) {
      LV = vector(mode = "numeric", length = maxNlines)
      locV = vector()
      for (i in 0:maxNlines) {
        CL = scan(ECSPathAndFile, what = character(), skip = i, 
                  nlines = 1, quiet = T)
        LV[i] = length(CL)
        ...
        if (i > nBlanks2Stop) {
          if (sum(LV[(i - nBlanks2Stop):i]) == 0) {
            if (verbose) 
              message(Sys.time(), " : ", nBlanks2Stop, 
                      " consecutive bank lines found in  ", 
                      ECSPathAndFile, ".  Stopped file reading at line number ", 
                      i)
            break
          }
        }
      ...
      if (length(locV) < 1) 
        warning(Sys.time(), " : No instances of calibration parameter", 
                ECSVar, " found in ", ECSPathAndFile)
      if (length(locV) > 0 & verbose) 
        message(Sys.time(), " : ", length(locV), " instances of calibration parameter", 
                ECSVar, " found in ", ECSPathAndFile)
      return(list(locV = locV, maxReadLine = i))
    }
    parameterValueF = function(ECSPathAndFile, loc) suppressWarnings(na.omit(as.numeric(scan(ECSPathAndFile, 
                                                                                             what = character(), skip = loc, nlines = 1, quiet = T)))[1])
    settingNameV = c("FILESET", "SOURCECAL", "LOCALCAL")
    settingLocV = vector(mode = "numeric", length = length(settingNameV))
    for (i in 1:length(settingLocV)) settingLocV[i] = finder(ECSPathAndFile = ECSPathAndFile, 
                                                             maxNlines = maxNlines, ECSVar = settingNameV[i])$locV
    ...
    if (length(freqLOC) < 1) 
      warning(Sys.time(), " : No echosounder frequencies found in ", 
              ECSPathAndFile)
    freqsetting = freqVal = vector(mode = "character", 
                                   length = length(freqLOC))
    for (i in 1:length(freqLOC)) {
      IND = lineDF$start[i] < freqLOC[i] & lineDF$stop[i] > 
        freqLOC[i]
    ...
    channel = paste(toupper(substr(channel, 1, 1)), substr(channel, 
                                                           2, nchar(channel) - 3), toupper(substr(channel, nchar(channel) - 
                                                                                                    2, nchar(channel) - 2)), substr(channel, nchar(channel) - 
                                                                                                                                      1, nchar(channel)), sep = "")
    ...
    #if (!is.null(FileNameSuffix)) 
    #  ECSPathAndFile.out = gsub(".ecs", paste(FileNameSuffix, 
    #                                          ".ecs", sep = ""), ECSPathAndFile)
    if (length(NewValue) != length(locV)) 
      stop("Length of NewValue ARG must equal number of ", 
           ECStargetVar, " calibration parameters found in ECS file.", 
           "\n", " Currently, ", length(locV), " instances of the calibration parameter ", 
           ECStargetVar, " were found \n  and the NewValue argument has ", 
           length(NewValue), " elements.")
    ...
    for (j in 0:(i - nBlanks2Stop)) {
      CL = scan(ECSPathAndFile, what = character(), skip = j, 
                nlines = 1, quiet = T)
      CL = gsub("\\s+", "", CL)
      if (j %in% locV) {
        message(Sys.time(), " : Modifying calibration parameter, ", 
                ECStargetVar, ".  Current value = ", parameterValueF(ECSPathAndFile, 
                                                                     j))
        message(Sys.time(), " : Modifying calibration parameter, ", 
                ECStargetVar, " with value ", NewValue[NewValCounter])
      ...
      write.table(paste(CL, collapse = " "), ECSPathAndFileOut, 
                  append = ifelse(j == 0, F, T), quote = F, sep = " ", 
